...
_G.UpdateTicketESP = function()
    ...
    -- Поиск тикетов по новому пути workspace.Effects.Tickets
    local effects = workspace:FindFirstChild("Effects")
    if effects then
        local tickets = effects:FindFirstChild("Tickets")
        if tickets then
            -- Ищем тикеты с именем "Visual"
            for _, ticket in pairs(tickets:GetChildren()) do
                if ticket:IsA("BasePart") or ticket:IsA("Model") then
                    -- Проверяем имя "Visual" или любое другое
                    if ticket.Name == "Visual" or ticket:FindFirstChild("Visual") then
