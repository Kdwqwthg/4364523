...
local function SafeHttpGet(url)
    ...
    if req then
        ...
        if ok and type(res) == "table" and type(res.Body) == "string" and #res.Body > 1000 then
            return res.Body
        end
    end
...
local function LoadLibrary(urls, name)
    local compiler = loadstring or load
    if type(compiler) ~= "function" then
        error(name .. ": loadstring/load is missing on this executor")
    end
    ...
    for _, url in ipairs(urls) do
        ...
        if not src then
            ...
            if not fn then
                ...
                lastErr = "runtime failed: " .. tostring(result)
            end
        end
    end

    error(name .. " load failed (" .. tostring(lastErr) .. ")")
end
...
local SaveManager = Fluent.SaveManager or env.SaveManager or SaveManager
local InterfaceManager = Fluent.InterfaceManager or env.InterfaceManager or InterfaceManager

if not SaveManager or not InterfaceManager then
    error("FluentPro managers missing (SaveManager/InterfaceManager)")
end
...
if type(floatingBtnSource) ~= "string" or #floatingBtnSource < 80 then
    floatingBtnSource = SafeHttpGet(FLOATING_BTN_URL) or game:HttpGet(FLOATING_BTN_URL, true)
end
local FloatingButton = (loadstring or load)(floatingBtnSource)()
...
Notify("Draconic X Evade", "System Loaded...", "Success", "solar/check-circle-bold", 3)
...
timerConnection = game:GetService("RunService").Heartbeat:Connect(function()
 ...
 MiscTab = Window:AddTab({ Title = "Misc", Icon = "solar/star-bold", Favoriteable = true })
...
VisualsTab = Window:AddTab({ Title = "Visuals", Icon = "solar/folder-bold", Favoriteable = true })
VisualsTab:AddSection("Cosmetics Changer", "solar/pallete-2-bold")
...
VisualsTab:AddSection("CarryAnimation Replacer", "solar/rewind-forward-bold")
...
VisualsTab:AddSection("Fake Streaks", "solar/fire-bold")
...
VisualsTab:AddSection("Emote Changer", "solar/emoji-funny-circle-bold")
...
VisualsTab:AddSection("Emote Replacer (very buggy)", "solar/danger-triangle-bold")
